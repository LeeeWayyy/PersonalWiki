        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            ci.atomic_write_json(
                invalid_path,
                {
                    ...
                    "section_label": "Chapter 2",
                    "prompt_version": ci.PROMPT_VERSION,
                    "error": "old formatting mismatch",
                    "raw_response": json.dumps(raw),
                },
            )

            recovered = ci.analyze_chapter(
                text,
                source_id=SOURCE_ID,
                source_sha256=SOURCE_SHA,
                section_label="Chapter 2",
                ...
                schema_ingest_path=schema,
                cache_dir=cache,
                complete=lambda *args, **kwargs: self.fail("recovery called LLM"),
            )
            self.assertEqual(
                recovered["claims"][0]["source_spans"][0]["quote"], text
            )
            self.assertEqual(json.loads(valid_path.read_text()), recovered)
            self.assertFalse(invalid_path.exists())

    def test_cache_can_be_adopted_after_failed_run_mints_new_source_id(self):
        old_source_id = "01K11111111111111111111111"
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            schema = root / "schema.md"
            schema.write_text("rules", encoding="utf-8")
            cache = root / "cache"
            artifact = artifact_for()
            artifact["source_id"] = old_source_id
            schema_sha = ci.selected_schema_digest(schema)
            key = ci.cache_key(
                source_sha256=SOURCE_SHA,
                text_sha256=ci.sha256_text(TEXT),
                section_label="Chapter 2",
            ...
            old_path = ci.cache_path(
                cache,
                prompt_version=ci.PROMPT_VERSION,
                source_id=old_source_id,
                key=key,
            )
            ci.atomic_write_json(old_path, artifact)

            adopted = ci.analyze_chapter(
                TEXT,
                source_id=SOURCE_ID,
                source_sha256=SOURCE_SHA,
                section_label="Chapter 2",
                ...
                schema_ingest_path=schema,
                cache_dir=cache,
                complete=lambda *args, **kwargs: self.fail("cache miss called LLM"),
            )
            self.assertEqual(adopted["source_id"], SOURCE_ID)
            new_path = ci.cache_path(
                cache,
                prompt_version=ci.PROMPT_VERSION,
                source_id=SOURCE_ID,
                key=key,
            )
            self.assertEqual(json.loads(new_path.read_text()), adopted)

    def test_corrupt_current_cache_adopts_valid_equivalent_sibling(self):
        old_source_id = "01K11111111111111111111111"
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            schema = root / "schema.md"
            ...
            artifact = artifact_for()
            artifact["source_id"] = old_source_id
            ci.write_cache_entry(old_path, artifact, inputs)
            current_path = ci.cache_path(
                cache,
                prompt_version=ci.PROMPT_VERSION,
                source_id=SOURCE_ID,
                key=key,
            ...
            self.assertEqual(adopted["source_id"], SOURCE_ID)
            self.assertEqual(json.loads(current_path.read_text()), adopted)
            self.assertEqual(ci.read_cache_entry(current_path)["artifact"], adopted)
